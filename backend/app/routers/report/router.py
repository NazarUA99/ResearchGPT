...
async def get_reports(
    skip: int = 0, limit: int = 10, agent_db_session=Depends(get_agent_db_session)
):
    """
    Get reports
    """
    ...
    return [
        ReportResponseModel(**result.model_dump())
        for result in await report_service.find_all(skip=skip, limit=limit)
    ]


@router.get("/{report_id}", response_model=ReportResponseModel)
async def get_report(report: ReportModel = Depends(get_report_by_id)):
    """
...
async def get_chunks(
    ...
    chunks = await chunk_service.find_chunks_by_report_id_and_type(
        report_id=report.uuid, skip=skip, limit=limit, type=type.value
    )
    return chunks
...
async def run_query(
    ...
    Example Responses:
        ...
        [
            {
                "type": "INTERNAL",
                "query": "Arts and Crafts Workshops conducted by Wellingtechinc Children’s Foundation funded by the Smith Family",
                "score": 3.1973040103912354,
                "source": "34130cb7-d720-4f17-a214-ff960a34a65b/sharepoint/Wellingtechinc Internal/Documents/Demo/Impact Reports/Quarterly Program Impact Summary.docx",
                "content": 'content',
    ...
    report_flow_service = ReportFlowService(
        ...
        langfuse_trace_args={
            ...
            "metadata": {"reportID": report_id, "request_params": model.model_dump()},
        },
    )
    results = await report_flow_service.run_custom_query(
        query=model.query, type=model.type
    )
...
async def upload_file(
    ...
    report_flow_service = ReportFlowService(
        tenant=tenant_model, report=report, db_session=agent_db_session
    )
...
async def generate_report(
    ...
    report_flow_service = ReportFlowService(
        ...
        langfuse_trace_args={
            ...
            "metadata": {"reportID": report.uuid, "request_params": model.model_dump()},
        },
    )

    if model.auto_select:
        internal_chunks = await chunk_service.find_chunks_by_report_id_and_type(
            report_id=report.uuid, type=ChunkTypeEnum.INTERNAL.value
        )
        web_chunks = await chunk_service.find_chunks_by_report_id_and_type(
            report_id=report.uuid, type=ChunkTypeEnum.WEB.value
        )
        logger.info("get chunks")
        return await report_flow_service.generate_report_from_chunks(
...
@router.post(
    "/{report_id}/chat/{session_id}", response_model=ChatWithReportResponseModel
)
async def chat_with_report(
    ...
    report_flow_service = ReportFlowService(
        ...
        langfuse_trace_args={
            ...
            "metadata": {"reportID": report.uuid, "session_id": session_id},
        },
    )
